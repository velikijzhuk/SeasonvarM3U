	                ...
	                <body class = "bg-light">
                        <div class = container">
							...
							<form class = "card p-2" method = "POST" action = "index.php">
  										...
  										<option selected disabled>Перевод...</option>
TRANS;
...
echo <<<TRANS
									...
									<select class = "form-select" aria-label = "Формат" name = "format_select">
										<option selected value = "1">Default</option>
                                                                                ...
                                                                                <option disabled value = "5">DL</option>
									</select>
...
if ($_POST['format_select'] == "2") {
	...
	foreach ($list_array as $ep_array) {
		...
		$m3u .= base64_decode($file).PHP_EOL;
	};
	$m3u .= PHP_EOL;
...
if ($_POST['format_select'] == "3") {
        ...
        $file = str_replace($garbage_angry, "", $file);
        $file = str_replace($garbage, "", $file);
                $txt .= base64_decode($file).PHP_EOL;
        };
        $txt .= PHP_EOL;
...
HTML;
	...
	echo "<center>";
        $file = str_replace($garbage_angry, "", $file);
        $file = str_replace($garbage, "", $file);
               echo "<a href=\"vlc-x-callback://x-callback-url/stream?url=".base64_decode($file)."\">".$title."</a><br>";
        };
echo <<< HOMEBTN
<div class = "py-4 text-center">
</div>
